...
- **Phase 3 — В ОСНОВНОМ СДЕЛАНА (остался инкремент 4).** Решение по старту (взвешено по хрупкости): **lazy-FIRST-start + БЕЗ idle-shutdown** — бридж стартует на первом `draw()` N-панели (`note_panel_visible` → heartbeat-таймер делает off-thread spawn; `launch_bridge` расщеплён: `resolve_launch_target`[main] + `spawn_bridge_process`[worker, без bpy] + `apply_launched_state`[main]; fire-once + failure-latch), живёт всю сессию, гаснет только parent-PID watchdog'ом (`BridgeLeaseWatchdogService` — уже был: owner-PID + grace для активных джобов) или graceful `unregister`. Закрыто:
  ...
  - **ОСТАЛОСЬ в Phase 3 = инкремент 4:** авто-reconnect с различением причин (flapping/BridgeMissing/CloudUnreachable) + **убрать ручную Refresh** из основного потока (tag_redraw уже делает её избыточной).
