...
- **Phase 5 — ИНКРЕМЕНТ A НАЧАТ (фундамент, НЕ закончен).** Добавлены пакеты `OutWit.Common.Settings` 2.0.2 + `.Json` 2.1.1 в бридж + класс `Configuration/BridgeRenderSettings.cs` (`SettingsContainer`; корзина 1: `RememberRenderSettings`, `SplitFrame`/`TilesX/Y/Overlap`, `AnimResult`, `VideoContainer/Codec`, `LastGroupId/Name`). Бридж собирается. Токен в settings НЕ кладём. **ОСТАЛОСЬ в A:** embedded `render-settings.json` (схема: per-group массивы `{key,value,valueKind,[tag],[hidden]}` + `__groups__`, как у клиента `Resources/settings.json`) + DI `builder.Services.AddSettings(s => s.UseJsonResource(asm,"render-settings.json").RegisterContainer<BridgeRenderSettings>())` (+ `WithFileName`, `Merge`/`Load` как в `ClientRuntimeBootstrap`) на per-user путь. **Потом:** B (REST get/set на `IBlenderBridgeChannel` + `BridgeRestRequestProcessor`), C (аддон: seed при коннекте + sticky на успешном submit + мастер-тоггл в UI; props = транзитный bpy-binding), D (баг `_scene_requires_upload` — добавить проверку формата → форсить re-upload при смене).
